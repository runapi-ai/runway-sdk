---
name: runway
description: "Generate or extend videos with Runway through RunAPI. Use the RunAPI CLI for one-off results and an SDK when integrating Runway into an app, backend, worker, library, or production codebase."
...
- For a one-off artifact or result, use the registered `runway` service in the `runapi` CLI. If the installed command catalog does not list it, stop and report the missing service instead of inventing a command.
...
runapi runway --help
runapi runway <operation> --help
curl --fail --location https://runapi.ai/docs/api/runway/<operation>.md --output contract.md
```
...
```shell
runapi runway <operation> --async --input-file request.json > task.json
...
```shell
runapi wait "$task_id" --service runway --action <operation> > result.json
```
...
- SDK integration: https://github.com/runapi-ai/runway-sdk
